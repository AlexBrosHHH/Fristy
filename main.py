...
class Assistant(QtWidgets.QMainWindow, interface.Ui_MainWindow, threading.Thread):
    ...
    def __init__(self):
        ...
        self.cmds = {
            ('текущее время', 'сейчас времени', 'который час', 'время', 'сколько времени'): self.time,
            ('привет', 'добрый день', 'здравствуй'): self.hello,
            ('пока', 'вырубись', 'выключись', 'стоп'): self.quite,
            ('выключи компьютер', 'выруби компьютер', 'завершение работы'): self.shut,
            ('какая погода', 'погода', 'погода на улице', 'сколько градусов'): self.weather,
